...
(defrecord RedisStore [conn default-serializer serializers compressor encryptor read-handlers write-handlers locks]
  ...
  (-put-raw-meta [this key binary]
    (let [res-ch (async/chan 1)]
      (async/thread
        (try
          (io/raw-update-meta conn (str-uuid key) binary)
          (async/close! res-ch)
  ...
  (-put-raw-value [this key binary]
    (let [res-ch (async/chan 1)]
      (async/thread
        (try
          (io/raw-update-it-only conn (str-uuid key) binary)
          (async/close! res-ch)
          (catch Exception e (async/put! res-ch (prep-ex "Failed to write raw value to store" e)))))
      res-ch)))

(defn new-redis-store
  ([]
   (new-redis-store {:pool {} :spec {}}))
  ([carmine-conn & {:keys [default-serializer serializers compressor encryptor read-handlers write-handlers]
    ...
    (let [res-ch (async/chan 1)]                      
      (async/thread 
        (try
          (async/put! res-ch
            (map->RedisStore {:conn carmine-conn
